...
// Hàm tìm kiếm nhị phân theo số điện thoại trong cây nhị phân
CenterPoint *binarySearch(CenterPoint *root, long long phone_num) 
{
    static int loop = 0;
    loop++;
    printf("\ntime loop: %d\n", loop);
    ...
    if (phone_num == root->phone_num) {
        return root; // Tìm thấy số điện thoại
    }

    if (phone_num < root->phone_num) {
        return binarySearch(root->left, phone_num);
    } else {
        return binarySearch(root->right, phone_num);
    }
}

// Hàm thêm nút mới vào danh sách liên kết có sắp xếp theo số điện thoại
void addNodeSorted(Node **head, char name[10], int age, char add[20], long long phone_num) 
{
    ...
    if (*head == NULL || phone_num < (*head)->phone_num) {
        new_node->next = *head;
        *head = new_node;
        return;
    }
    ...
    Node *current = *head;
    while (current->next != NULL && current->next->phone_num < phone_num) {
...
void inOrderPrint(CenterPoint *root) 
{
    ...
    printf("name: %s, add: %s, age: %d, phone number: %lld\n", 
...
int main() 
{
    ...
    // In cây để kiểm tra
    printf("buildTree with phone number:\n");
    inOrderPrint(root);

    // Tìm kiếm số điện thoại trong cây
    long long search_phone = 44332211002;

    CenterPoint *result = binarySearch(root, search_phone);
    ...
    if (result != NULL) {
        ...
        printf("don't see phone number: %lld\n", search_phone);
    }
